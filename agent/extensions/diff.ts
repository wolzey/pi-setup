...
const editorCommand = process.env.PI_DIFF_EDITOR ?? "nvim";

function editorArgs(file: string) {
  return editorCommand.split(/\s+/).filter(Boolean).concat(file);
}
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("agent_end", async (_event, ctx) => {
    ...
    if (changedFiles.size > 0) {
      ctx.ui.notify(`${changedFiles.size} changed file(s). Run /${commandName} to view/open in Neovim.`, "info");
    }
  });

  pi.registerCommand(commandName, {
    description: "Show files changed by the last agent run and open one in Neovim",
    handler: async (args, ctx) => {
      ...
      const selected = await ctx.ui.select("Open changed file in Neovim", labels);
      if (!selected) return;
      ...
      if (result.code === 0) {
        ctx.ui.notify(`Opened ${selected} in ${editorCommand} in a new WezTerm window`, "info");
      } else {
        ctx.ui.notify(result.stderr.trim() || `Failed to open ${selected} in ${editorCommand}`, "error");
      }
    },
  });
}
